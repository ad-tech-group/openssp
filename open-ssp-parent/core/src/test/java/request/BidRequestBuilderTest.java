...
public class BidRequestBuilderTest {
	...
	public static void setUp() {
		bidRequest = new BidRequest.Builder().setId("1234U8Bc3").setSite(new Site.Builder().setId("1234").setDomain("domain").addCat("IAB1").addCat("IAB11").setName(
		        "mySiteObject").setPage("page").build()).setDevice(new Device.Builder().setGeo(new Geo.Builder().setCity("Hamburg").setCountry("DEU").setLat(53.563452f).setLon(
		                9.925742f).setZip("22761").build()).build()).addImp(new Impression.Builder().setId("45").setVideo(new Video.Builder().addMime(
		                        "application/x-shockwave-flash").setH(400).setW(600).setMaxduration(100).setMinduration(30).addProtocol(VideoBidResponseProtocol.VAST_2_0).setStartdelay(1).build()).build()).setUser(new User.Builder().setBuyeruid("HHcFrt-76Gh4aPl").setGender(Gender.MALE).setId("99")
	...
	public void testBidRequest() {
		...
		Assert.assertEquals("IAB11", bidRequest.getSite().getCat().get(1));
		...
		//TODO: BKS
//		Assert.assertEquals(1, bidRequest.getImpX().get(0).getVideo().getProtocols().size());
//		Assert.assertEquals(2, bidRequest.getImpX().get(0).getVideo().getProtocols().get(0).intValue());
	...
	@Test
	public void testValidator1_0() throws IOException {
	...
	@Test
	public void testValidator2_0() throws IOException {
		final String jsonBidrequest = gson.toJson(bidRequest.getBuilder().build(), BidRequest.class);
	...
	@Test
	public void testValidator2_1() throws IOException {
		final String jsonBidrequest = gson.toJson(bidRequest.getBuilder().build(), BidRequest.class);
	...
	@Test
	public void testValidator2_2() throws IOException {
		final String jsonBidrequest = gson.toJson(bidRequest.getBuilder().build(), BidRequest.class);
	...
	@Test
	public void testValidator2_3() throws IOException {
		final String jsonBidrequest = gson.toJson(bidRequest.getBuilder().build(), BidRequest.class);
	...
	@Test
	public void testValidator2_4() throws IOException {
		final String jsonBidrequest = gson.toJson(bidRequest.getBuilder().build(), BidRequest.class);
